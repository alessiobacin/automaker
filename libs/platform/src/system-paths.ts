...
function hasProviderOAuth(authJson: Record<string, unknown>): boolean {
  for (const provider of OPENCODE_PROVIDERS) {
    ...
    if (providerAuth && typeof providerAuth === 'object' && !Array.isArray(providerAuth)) {
      ...
      // Check for OAuth type with access token or refresh token (OpenCode often stores refresh token)
      const isOAuthType = auth.type === 'oauth';
      const hasTokens =
...
function hasProviderApiKey(authJson: Record<string, unknown>): boolean {
  for (const provider of OPENCODE_PROVIDERS) {
    ...
    if (providerAuth && typeof providerAuth === 'object' && !Array.isArray(providerAuth)) {
      ...
      const isApiKeyType = auth.type === 'api' || auth.type === 'api_key';
      const hasKey = typeof auth.key === 'string' && auth.key && auth.key !== 'opencode auth login';

      if (isApiKeyType && hasKey) {
        return true;
      }
...
export async function getOpenCodeAuthIndicators(): Promise<OpenCodeAuthIndicators> {
  ...
  try {
    ...
    try {
      ...
      // Collect all provider names that have valid auth
      for (const provider of OPENCODE_PROVIDERS) {
        const providerAuth = authJson[provider];
        if (providerAuth && typeof providerAuth === 'object' && !Array.isArray(providerAuth)) {
          const auth = providerAuth as Record<string, unknown>;
          const hasTokens =
            ...
            (typeof auth.access_token === 'string' && auth.access_token) ||
            (typeof auth.refresh === 'string' && auth.refresh) ||
            (typeof auth.key === 'string' && auth.key && auth.key !== 'opencode auth login') ||
