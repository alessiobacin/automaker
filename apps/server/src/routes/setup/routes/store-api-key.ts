  ...
  return async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      if (provider === 'anthropic' || provider === 'anthropic_oauth_token') {
        ...
        await persistApiKeyToEnv('OPENROUTER_API_KEY', apiKey);
        logger.info('[Setup] Stored API key as OPENROUTER_API_KEY');
      } else {
        res.status(400).json({
          success: false,
          error: `Unsupported provider: ${provider}. Supported: anthropic, openai, openrouter.`,
        });
        return;
      }
