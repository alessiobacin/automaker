...
export function PhaseModelSelector({
  ...
  // Group models including separate OpenRouter
  const { favorites, claude, cursor, codex, opencode, openrouter } = React.useMemo(() => {
    const favs: typeof CLAUDE_MODELS = [];
    const cModels: typeof CLAUDE_MODELS = [];
    const curModels: typeof CURSOR_MODELS = [];
    const codModels: typeof CODEX_MODELS = [];
    const ocModels: typeof OPENCODE_MODELS = [];
    const orModels: typeof OPENCODE_MODELS = [];
    ...
    // Process OpenCode Models - separate OpenRouter from native OpenCode
        ...
        >
          ...
          {openrouter.length > 0 && (
            <CommandGroup heading="OpenRouter Models">
              {openrouter.map((model) => renderOpencodeModelItem(model))}
            </CommandGroup>
          )}
        </CommandList>
      </Command>
